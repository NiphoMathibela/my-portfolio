...
const ProjectCard = (props) => {
  ...
  let git = props.git
  return (
    <div className="w-full sm:w-[42%] h-[470px] rounded-xl m-4 drop-shadow-4xl bg-black pointer-events-auto py-2 ">
      ...
      <div className="m-1 text-white text-sm mx-auto p-5">
        <p className=" h-24 my-2 overflow-y-auto ">{props.description}</p>
        ...
        <div className="mr-6"><IoLogoGithub size={30} onClick={() => window.open(git)}/></div>
        <div onClick= {() => window.open(url)}><IoLink size={32}/></div>
        </div>
      </div>
    </div>
  );
